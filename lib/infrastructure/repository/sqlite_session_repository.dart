...
class SQLiteSessionRepository implements SessionRepository {
  ...
  Future<Session?> getSession() async {
    try {
      final List<Map<String, dynamic>> response = await _database.rawQuery(
        'SELECT * FROM Session WHERE endTime IS NULL',
      );
  ...
  Future<int> getMaxID(Transaction txc) async {
    try {
      final List<Map<String, dynamic>> maxIdResponse = await txc.rawQuery(
        'SELECT MAX(id) as maxId FROM Session',
      );
  ...
  Future<Session> updateRound(Session session, Transaction txc) async {
    try {
      ...
      await txc.rawUpdate(
        'UPDATE Session SET round = ? WHERE id = ?',
  ...
  Future<void> updateGameType(Session session, String gameType) async {
    try {
      await _database.rawUpdate(
        'UPDATE Session SET gameType = ? WHERE id = ?',
        [gameType, session.id],
      );
    } catch (e) {
